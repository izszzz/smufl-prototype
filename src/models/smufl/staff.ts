...
export class Staff {
	...
	constructor(glyph: SMUFL.Glyph | SMUFL.Ligature, track: SMUFL.Track, options?: ConstructorOptions)
	constructor(firstArg: (SMUFL.Glyph | SMUFL.Ligature) | number, track: SMUFL.Track, options?: ConstructorOptions){
		this.track = track
		const width =  R.isNumber(firstArg) ? firstArg : firstArg.width
		if(!R.isNumber(firstArg))this.glyph = firstArg
		this.staffGlyph = new SMUFL.Glyph(SMUFL.Staff.getStaffGlyph(width, track.staffLineCount)?.key)
	...
	static getStaffGlyph = (width: number, lineCount: Metadata["staffLines"][number] = 5) => {
		const glyph = R.pipe(
