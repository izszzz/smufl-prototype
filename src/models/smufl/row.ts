...
export class Row {
	...
	//  height: number;
	 ...
	 generateBarlines(tracks: SMUFL.Track[]){
		const generateBarlineEnd = (bar: SMUFL.Bar) =>  new SMUFL.Barline(R.isDefined(bar.barline.end) ? bar.barline.end.glyphName : bar.barline.start.glyphName)
		...
		lastBarline.glyph.x = firstTrack.bars.reduce((acc, cur) => acc + cur.width, 0)
